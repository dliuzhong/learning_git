...
for quizNum in range(35):
    # Create the quiz and answer key fils.
    quizFile = open('./FileOperate/Quiz/QuestionFiles/capitalsquiz%s.txt' % (quizNum + 1), 'w')
    answerKeyFile = open('./FileOperate/Quiz/AnswerFiles/capitalsquiz_answers%s.txt' % (quizNum + 1), 'w')
    ...
    random.shuffle(states)      # 打乱
    
    # Loop throuth all 50 states, making a question for each.
    for questionNum in range(50):

        #Get right and wrong answers.
        correctAnswer = capitals[states[questionNum]]
        wrongAnswers = list(capitals.values())
        del wrongAnswers[wrongAnswers.index(correctAnswer)] # 把正确的去掉
        wrongAnswers = random.sample(wrongAnswers, 3)   # 取三个
        answerOptions = wrongAnswers + [correctAnswer]
        random.shuffle(answerOptions)

        # Write the question and the answer to the quiz file.
        quizFile.write('%s. Whate is the catiptal of %s?\n' % (questionNum + 1, states[questionNum]))
        for i in range(4):
